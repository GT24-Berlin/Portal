...
export async function POST(
  ...
  if (assignment.status === 'ACCEPTED') {
    return NextResponse.json({
      ok: true,
      assignment: { id: assignment.id, status: 'ACCEPTED' }
    });
  }

  if (assignment.status !== 'PENDING') {
    return NextResponse.json(
      { ok: false, error: `Cannot accept when status=${assignment.status}` },
      { status: 409 }
    );
  }
  ...
  const updated = await prisma.caseAssignment.update({
    where: { id: assignment.id },
    data: { status: 'ACCEPTED' as any, acceptedAt: now },
    select: { id: true, status: true, active: true, expiresAt: true }
  });

  // ---- Notification (für den User, der akzeptiert hat) ----
  await prisma.notification.create({
    data: {
      userId,
      type: NotificationType.ASSIGNMENT_ACCEPTED,
      title: 'Zuweisung angenommen',
      body: `Du hast den Fall als ${role} angenommen.`,
      href: '/dashboard/inbox',
      caseId,
      role: role as any,
      readAt: null
  ...
  try {
    ...
    await sendMail({
      to: adminEmail,
      subject: `Gutachtery24 – Fall angenommen (${role}) – ${caseLabel}`,
      text:
    ...
    console.error('Email send failed (ASSIGNMENT_ACCEPTED):', e);
  }
